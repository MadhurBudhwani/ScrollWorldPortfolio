...
function chapterTransition(index,p,pose){
  ...
  Object.assign(hole.style,{left:pose.x+'px',top:(state.h-82)+'px',opacity:String(opacity)});
...
function revisionPose(index,p){
  ...
  // Finish the train journey on its last carriage, before the boundary portal opens.
  if(index===1&&p>.85){const g=trainGeometry(.85);return {...pose,x:state.w*.70,feet:g.y-100*g.scale,mode:'idle'};}
  return pose;
}
function renderForeground(pose,transition,dt){
  ...
  if(state.scene===2&&!transition.mode){
    const hand=avatar?.getHandAnchor(),r=stage.getBoundingClientRect();
    if(hand){
      const x=hand.x-r.left,y=hand.y-r.top,pulleyY=Math.min(state.h*.48,y-30),reelX=state.w*(state.w<1000?.52:.72);
      frontCtx.strokeStyle='#bdb49b';frontCtx.lineWidth=3;frontCtx.beginPath();frontCtx.moveTo(reelX,pulleyY+22);frontCtx.lineTo(reelX,pulleyY);frontCtx.lineTo(x,pulleyY);frontCtx.lineTo(x,y);frontCtx.stroke();
      frontCtx.strokeStyle='#67daf5';frontCtx.lineWidth=4;frontCtx.strokeRect(x-8,pulleyY-8,16,16);
      for(let dy=pulleyY+12-(state.directedLocal*200)%16;dy<y-4;dy+=16){frontCtx.fillStyle='#81795f';frontCtx.fillRect(x-2,dy,4,3);}
    }
  }
