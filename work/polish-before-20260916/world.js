...
function resize(){state.w=innerWidth;state.h=innerHeight;canvas.width=innerWidth;canvas.height=innerHeight;ctx.imageSmoothingEnabled=false;}
function background(){
...
let last=0;
function frame(t){const dt=Math.min((t-last)/1000||1/60,.05);last=t;background();avatar?.update(dt);requestAnimationFrame(frame);}
